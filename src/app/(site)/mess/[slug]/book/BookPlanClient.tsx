...
import styles from "./BookPlanClient.module.css";
...
export default function BookPlanClient() {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      ...
      const bookingPath = query ? `${pathname}?${query}` : pathname;
  ...
  useEffect(() => {
    if (!planIdParam) { toast.error("No plan selected."); router.back(); return; }
    setPlanLoading(true);
