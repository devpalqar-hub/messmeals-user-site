...
import "./SubscriptionActionModal.css";
...
export default function SubscriptionActionModal({ mode, messName, onClose, onConfirm }: Props) {
  ...
  return (
    <div className="sam-overlay" onClick={onClose}>
      <div className="sam-card" onClick={(e) => e.stopPropagation()}>
        <button className="sam-close" onClick={onClose}>
          <X size={18} />
        </button>

        <div className={`sam-icon ${tone}`}>
          <Icon size={24} />
        </div>

        <h3>{title}</h3>
        <p className="sam-sub">{messName}</p>
        ...
        {mode === "skip" && (
          <div className="sam-field">
            ...
            <p className="sam-note">Only this day's meal will be cancelled — your plan continues after.</p>
          </div>
        )}
        ...
        <div className="sam-actions">
          <button className="sam-cancel-btn" onClick={onClose}>
            Go back
          </button>
          <button
            className={`sam-confirm-btn ${tone}`}
